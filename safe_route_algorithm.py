    ...
    while open_set:
        ...
        for neighbor in get_neighbors(current, grid_size):
            ...
            if neighbor not in g_score or tentative_g < g_score[neighbor]:
                ...
                f_score = tentative_g + haversine(neighbor[0], neighbor[1], end.lat, end.lon)
...
def count_hazards(path: List[Dict[str, float]], hazards: List[Hazard]) -> int:
    ...
    for hz in hazards:
        ...
        for i in range(len(path) - 1):
            ...
            if haversine(p1[0], p1[1], center[0], center[1]) <= radius or \
               haversine(p2[0], p2[1], center[0], center[1]) <= radius:
                count += 1
                break
